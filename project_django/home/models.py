...
class Analist(models.Model):
    analist_id = models.BigAutoField(primary_key=True)
    name = models.CharField(max_length=50)
    lastn = models.CharField(max_length=50)
    sector = models.CharField(max_length=50, default='Sistemas')
    def __str__(self):
        ana = str(self.analist_id) + ', ' + self.name + ' ' + self.lastn + ', ' + str(self.sector)
        return ana
...
class Incident(models.Model):
    ...
    analist_id = models.ForeignKey(Analist, on_delete=models.CASCADE)
    ...
    def __str__(self):
        incident = 'Incident ID: ' + str(self.incident_id) + ', Issue: ' + self.desc + ', Affected User: ' + Usuario.objects.get(usuario_id = self.usuario_id) + ', Assigned Analist: ' + Analist.objects.get(analist_id = self.analist_id)
        return incident
